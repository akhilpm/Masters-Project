...
def arc_cosine_vector(X, Y):
	...
	M = X.dot(Y.T).todense()
	temp1 = X.dot(X.T).diagonal()   #np.diag(np.dot(X, X.T))
	temp2 = Y.dot(Y.T).diagonal()   #np.diag(np.dot(Y, Y.T))	

	#print(M.shape, temp1.shape, temp2.shape)
